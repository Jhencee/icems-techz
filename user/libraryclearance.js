...
async function fetchLibraryClearanceStatus(studentNumber, forceRefresh = false) {
    if (!forceRefresh && libraryStatusCache.has(studentNumber)) {
        console.log('ÃƒÆ’Ã‚Â°Ãƒâ€¦Ã‚Â¸ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œÃƒâ€šÃ‚Â¦ Using cached library status for:', studentNumber);
        return libraryStatusCache.get(studentNumber);
    }

    if (libPendingRequests.has(studentNumber)) {
        console.log('ÃƒÆ’Ã‚Â¢Ãƒâ€šÃ‚ÂÃƒâ€šÃ‚Â³ Waiting for existing request for:', studentNumber);
        ...
        try {
            ...
            if (response.ok && data.success) {
                if (data.exists && data.clearance) {
                    console.log('ÃƒÆ’Ã‚Â¢Ãƒâ€¦Ã¢â‚¬Å“ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¦ Library clearance found:', data.clearance.status);
                    result = {
                    ...
                    console.log('ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¾Ãƒâ€šÃ‚Â¹ÃƒÆ’Ã‚Â¯Ãƒâ€šÃ‚Â¸Ãƒâ€šÃ‚Â No library clearance submitted yet');
                    result = {
                ...
                console.log('ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¾Ãƒâ€šÃ‚Â¹ÃƒÆ’Ã‚Â¯Ãƒâ€šÃ‚Â¸Ãƒâ€šÃ‚Â Unexpected response, treating as not submitted');
                result = {
                    status: 'not_submitted',
                    remarks: '',
                    exists: false
            ...
            console.log('ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¾Ãƒâ€šÃ‚Â¹ÃƒÆ’Ã‚Â¯Ãƒâ€šÃ‚Â¸Ãƒâ€šÃ‚Â Could not fetch clearance status (network error)');
            result = {
                status: 'not_submitted',
                remarks: '',
                exists: false
                    ...
                    <div style="margin-bottom: 25px; padding: 15px; background: #f0f9ff; border-radius: 8px; border: 2px solid #0ea5e9;">
                        ...
                        <div style="display: flex; flex-direction: column; gap: 12px;">
                            <label style="display: flex; align-items: start; cursor: pointer; padding: 12px; background: white; border-radius: 6px; border: 2px solid #e0e0e0; transition: all 0.2s;">
                                ...
                                <div>
                                    <strong style="display: block; color: #800020;">ÃƒÆ’Ã‚Â°Ãƒâ€¦Ã‚Â¸ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œÃƒâ€šÃ‚Â¤ Submit Online</strong>
                            ...
                            <label style="display: flex; align-items: start; cursor: pointer; padding: 12px; background: white; border-radius: 6px; border: 2px solid #e0e0e0; transition: all 0.2s;">
                                ...
                                <div>
                                    <strong style="display: block; color: #800020;">ÃƒÆ’Ã‚Â°Ãƒâ€¦Ã‚Â¸Ãƒâ€šÃ‚ÂÃƒâ€šÃ‚Â¢ Direct Library Visit</strong>
...
function addLibraryBookItem() {
    ...
    const itemHTML = `
        <div class="library-book-item" id="library_book_${itemId}" style="background: white; padding: 15px; border-radius: 8px; margin-bottom: 15px; border: 2px solid #e0e0e0; position: relative;">
            <button type="button" onclick="removeLibraryBookItem(${itemId})" style="position: absolute; top: 10px; right: 10px; background: #dc2626; color: white; border: none; border-radius: 50%; width: 25px; height: 25px; cursor: pointer; font-size: 0.9rem;">ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â</button>
...
async function handleLibrarySubmit(e) {
    ...
    if (clearanceType === 'direct_visit') {
        ...
        try {
            ...
            if (data.success) {
                alert('ÃƒÆ’Ã‚Â¢Ãƒâ€¦Ã¢â‚¬Å“ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¦ Direct visit request submitted successfully!\n\nPlease visit the library during operating hours:\nMonday to Friday: 8:00 AM - 5:00 PM\nLocation: University Library, 2nd Floor');
                closeLibraryModal();
                clearLibraryCache(currentUser.student_number);
                location.reload();
            } else {
                alert('ÃƒÆ’Ã‚Â¢Ãƒâ€šÃ‚ÂÃƒâ€¦Ã¢â‚¬â„¢ ' + (data.message || 'Failed to submit request'));
            }
        } catch (error) {
            console.error('Error submitting direct visit request:', error);
            alert('ÃƒÆ’Ã‚Â¢Ãƒâ€šÃ‚ÂÃƒâ€¦Ã¢â‚¬â„¢ Failed to submit request. Please try again.');
        } finally {
    ...
    try {
        ...
        if (data.success) {
            alert('ÃƒÆ’Ã‚Â¢Ãƒâ€¦Ã¢â‚¬Å“ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¦ Library clearance submitted successfully!\n\nYour submission is pending admin approval.');
            closeLibraryModal();
            clearLibraryCache(currentUser.student_number);
            location.reload();
        } else {
            alert('ÃƒÆ’Ã‚Â¢Ãƒâ€šÃ‚ÂÃƒâ€¦Ã¢â‚¬â„¢ ' + (data.message || 'Failed to submit clearance'));
        }

    } catch (error) {
        console.error('Error submitting library clearance:', error);
        alert('ÃƒÆ’Ã‚Â¢Ãƒâ€šÃ‚ÂÃƒâ€¦Ã¢â‚¬â„¢ Failed to submit clearance. Please try again.');
    } finally {
        submitBtn.disabled = false;
        submitBtn.innerHTML = originalBtnText;
    }
...
document.addEventListener('DOMContentLoaded', async function() {
    console.log('ÃƒÆ’Ã‚Â°Ãƒâ€¦Ã‚Â¸ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œÃƒâ€¦Ã‚Â¡ Library module: DOMContentLoaded fired');
    console.log('ÃƒÆ’Ã‚Â°Ãƒâ€¦Ã‚Â¸ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œÃƒâ€¦Ã‚Â¡ Current path:', window.location.pathname);
    
    if (window.location.pathname.includes('studentclearance.html')) {
        console.log('ÃƒÆ’Ã‚Â°Ãƒâ€¦Ã‚Â¸ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œÃƒâ€¦Ã‚Â¡ Loading library clearance card...');
        await addLibraryClearanceCard();
        console.log('ÃƒÆ’Ã‚Â°Ãƒâ€¦Ã‚Â¸ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œÃƒâ€¦Ã‚Â¡ Library clearance card loaded');
    } else {
        console.log('ÃƒÆ’Ã‚Â°Ãƒâ€¦Ã‚Â¸ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œÃƒâ€¦Ã‚Â¡ Not on clearance page, skipping');
    }
});
