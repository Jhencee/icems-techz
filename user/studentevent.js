...
function formatEventDateTime(dateStart, dateEnd) {
    ...
    return `${dateStart.toLocaleDateString('en-US', options)} | ${dateStart.toLocaleTimeString('en-US', timeOptions)} - ${dateEnd.toLocaleTimeString('en-US', timeOptions)}`;
}

function getTimeRemaining(dateStart) {
    const now = new Date();
...
function createEventBackFaceHtml(event) {
    return `<p style="font-weight:600;font-size:0.8rem;color:#800020;">${event.title}</p><p style="font-size:0.75rem;">${event.location}</p>`;
}
        ...
        <div class="modal active" id="calendarEventModal" style="display: flex; z-index: 10000;">
            <div class="modal-content" style="max-width: 600px;">
                ...
                <div style="text-align: center; margin-bottom: 20px;">
                    <div style="display: inline-block; background: ${categoryColor}; color: white; padding: 8px 20px; border-radius: 20px; font-size: 0.85rem; font-weight: 700; margin-bottom: 15px;">${categoryLabel}</div>
                    <h2 class="modal-title" style="color: #800020; margin: 0;">${event.title}</h2>
                </div>
                <div style="background: #f9fafb; padding: 20px; border-radius: 10px; margin-bottom: 20px;">
                    <div style="margin-bottom: 12px;"><i class="fas fa-calendar" style="color: #800020; width: 25px;"></i><strong>${formattedDate}</strong></div>
                    <div style="margin-bottom: 12px;"><i class="fas fa-clock" style="color: #800020; width: 25px;"></i>${timeStart} - ${timeEnd}</div>
                    <div style="margin-bottom: 12px;"><i class="fas fa-map-marker-alt" style="color: #800020; width: 25px;"></i>${event.location}</div>
                    <div style="margin-bottom: 12px;"><i class="fas fa-users" style="color: #800020; width: 25px;"></i>${event.audience}</div>
                    <div><i class="fas fa-user-tie" style="color: #800020; width: 25px;"></i>Organized by: ${event.organizer}</div>
                </div>
                ...
                <div style="display: flex; gap: 10px; justify-content: center; border-top: 1px solid #e5e7eb; padding-top: 20px;">
                    <button class="btn btn-primary" onclick="closeCalendarEventModal(); openUploadModal('${event.id}', '${event.title.replace(/'/g, "\\'")}');" style="flex: 1;"><i class="fas fa-upload"></i> Upload Proof</button>
                    <button class="btn btn-secondary" onclick="closeCalendarEventModal()" style="flex: 1;">Close</button>
                </div>` : `
...
function openUploadModal(eventId, eventTitle) {
    ...
    selectedFile = null;
    document.getElementById('modalEventTitle').textContent = 'Submit Attendance Proof: ' + eventTitle;
...
async function submitProof() {
    ...
    try {
        ...
        btn.disabled = false;
        btn.innerHTML = '<i class="fas fa-upload"></i> Submit Proof';
    }
}
...
function saveProfile() { alert('Profile updated successfully!'); closeProfileModal(); }
function toggleSidebar() { document.getElementById('sidebar').classList.toggle('active'); }
function logout() { document.getElementById('logoutModal').style.display = 'flex'; }
function closeLogoutModal() { document.getElementById('logoutModal').style.display = 'none'; }
function confirmLogout() { localStorage.removeItem('currentUser'); window.location.href = 'studentlogin.html'; }
