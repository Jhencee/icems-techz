...
async function loadEventsFromDatabase() {
    try {
        const response = await fetch(`${API_BASE_URL}/events?admin=`+encodeURIComponent(JSON.parse(localStorage.getItem('currentUser')||'{}').name||'')+``);
        ...
        if (data.success) {
            ...
            console.log('Ã¢Å“â€¦ Loaded', data.events.length, 'events from database');
        }
    } catch (error) {
        console.error('Ã¢ÂÅ’ Failed to load events:', error);
...
async function createEventInDatabase(eventData) {
    try {
        ...
        if (data.success) {
            console.log('Ã¢Å“â€¦ Event created in database:', data.event);
            return data.event;
        } else {
            throw new Error(data.message);
        }
    } catch (error) {
        console.error('Ã¢ÂÅ’ Failed to create event:', error);
        throw error;
    }
}
...
async function updateEventInDatabase(eventId, eventData) {
    try {
        ...
        if (data.success) {
            console.log('Ã¢Å“â€¦ Event updated in database:', data.event);
            return data.event;
        } else {
            throw new Error(data.message);
        }
    } catch (error) {
        console.error('Ã¢ÂÅ’ Failed to update event:', error);
        throw error;
    }
}
...
async function deleteEventFromDatabase(eventId) {
    try {
        ...
        if (data.success) {
            console.log('Ã¢Å“â€¦ Event deleted from database');
            return true;
        } else {
            throw new Error(data.message);
        }
    } catch (error) {
        console.error('Ã¢ÂÅ’ Failed to delete event:', error);
        throw error;
    }
}
...
function showSection(sectionId) {
    ...
    // Ã¢Å“Â¨ FIX: Load clearance submissions when clearance section is shown
    if (sectionId === 'clearance') {
        console.log('Ã°Å¸â€œâ€¹ Loading clearance section...');
...
async function loadClearanceSubmissions() {
    console.log('Ã°Å¸â€Â Loading clearance submissions from:', `${API_BASE_URL}/clearance/submissions`);
    ...
    try {
        ...
        const data = await response.json();
        console.log('Ã°Å¸â€œÂ¦ Raw API response:', data);
        console.log('Ã°Å¸â€œÅ  Stats breakdown:');
        ...
        if (data.success) {
            ...
            console.log('Ã¢Å“â€¦ Loaded', clearanceSubmissions.length, 'submissions');
            ...
            console.error('Ã¢ÂÅ’ API returned success: false', data.message);
            showEmptyClearanceState('Server returned an error: ' + (data.message || 'Unknown error'));
        }
    } catch (error) {
        console.error('Ã¢ÂÅ’ Error loading submissions:', error);
...
function displayClearanceSubmissions(submissions) {
    ...
    if (!clearanceSection) {
        console.error('Ã¢ÂÅ’ Clearance table not found');
        return;
    }
...
function getStatusBadge(status) {
    switch (status) {
        case 'pending':
            return '<span style="color:#f59e0b; font-weight: 600;">Ã¢ÂÂ³ PENDING</span>';
        case 'approved':
            return '<span style="color:#059669; font-weight: 600;">Ã¢Å“â€¦ APPROVED</span>';
        case 'rejected':
            return '<span style="color:#dc2626; font-weight: 600;">Ã¢ÂÅ’ REJECTED</span>';
        default:
...
function updateClearanceStats(data) {
    console.log('Ã°Å¸â€œÅ  Updating clearance stats:', data);
    ...
    for (const [elementId, value] of Object.entries(clearanceStats)) {
        ...
        if (element) {
            ...
            console.log(`  Ã¢Å“â€¦ ${elementId}: ${value}`);
        }
    }
    ...
    if (dashboardTotalElement) {
        ...
        console.log(`  Ã¢Å“â€¦ dashboardTotalClearance: ${data.total || 0}`);
    }
    ...
    if (dashboardPendingElement) {
        ...
        console.log(`  Ã¢Å“â€¦ dashboardPendingClearance: ${data.pending || 0}`);
    }

    console.log('Ã¢Å“â€¦ Stats update complete');
}
...
async function approveSubmission(submissionId) {
    ...
    try {
        ...
        if (data.success) {
            alert('Ã¢Å“â€¦ Submission approved successfully!');
            closeSubmissionModal();
            await loadClearanceSubmissions();
        } else {
            alert('Ã¢ÂÅ’ ' + (data.message || 'Failed to approve submission'));
        }
    } catch (error) {
        console.error('Error approving submission:', error);
        alert('Ã¢ÂÅ’ Failed to approve submission. Please try again.');
    }
}
...
async function rejectSubmission(submissionId) {
    ...
    try {
        ...
        if (data.success) {
            alert('Ã¢ÂÅ’ Submission rejected.');
            closeSubmissionModal();
            await loadClearanceSubmissions();
        } else {
            alert('Ã¢ÂÅ’ ' + (data.message || 'Failed to reject submission'));
        }
    } catch (error) {
        console.error('Error rejecting submission:', error);
        alert('Ã¢ÂÅ’ Failed to reject submission. Please try again.');
    }
}
...
document.addEventListener('DOMContentLoaded', async () => {
    console.log('Ã°Å¸Å¡â‚¬ Initializing SSO Dashboard...');
    ...
    console.log('Ã¢Å“â€¦ SSO Dashboard initialized successfully');
});
