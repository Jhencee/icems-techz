...
async function updateAdminPassword(adminId, newPassword, adminName) {
    try {
        ...
        // ✅ FIX 1: Attach Bearer token if stored (token-based auth)
        ...
        // ✅ FIX 2: Attach CSRF token for Laravel session-based auth
        ...
        const response = await fetch(`${apiUrl}/admin/reset-password`, {
            method: 'POST',
            headers: headers,
            credentials: 'include', // ✅ FIX 3: Send session cookies with request
            body: JSON.stringify({
        ...
        if (error.message.includes('401')) {
            alertFn(
                'Error',
                'Session expired or not logged in. Please log in as Super Admin and try again.',
                'error'
            );
