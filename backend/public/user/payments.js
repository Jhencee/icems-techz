...
function updateStatistics() {
  ...
  document.querySelector('.stat-card:nth-child(1) .stat-value').textContent = `â‚±${totalPaid.toLocaleString()}`;
  document.querySelector('.stat-card:nth-child(2) .stat-value').textContent = `â‚±${totalUnpaid.toLocaleString()}`;
  ...
  container.innerHTML = unpaidRequirements.map(req => {
    ...
    return `
      <div class="payment-card unpaid" data-status="pending">
        ...
        <div class="payment-amount">â‚±${parseFloat(req.amount).toLocaleString()}</div>
          ...
          ${sortedPayments.map(payment => {
            ...
            return `
              <tr style="border-bottom: 1px solid #e5e7eb;">
                ...
                <td style="padding: 12px;">â‚±${parseFloat(payment.amount).toLocaleString()}</td>
...
async function submitPayment() {
  ...
  try {
    ...
    if (data.success) {
      alert(`âœ… Payment submitted for ${currentPayment.name} (â‚±${currentPayment.amount}).\nYour payment is now being verified.`);
      closePaymentModal();
...
function updateStatistics() {
  ...
  if (elPaid) elPaid.textContent = `â‚±${totalPaid.toLocaleString()}`;
  if (elUnpaid) elUnpaid.textContent = `â‚±${totalDue.toLocaleString()}`;
  ...
  container.innerHTML = `
    ...
    return `
                            <tr style="border-bottom:1px solid #e5e7eb;">
                                ...
                                <td style="padding:12px;">â‚±${parseFloat(p.amount).toLocaleString()}</td>
                                <td style="padding:12px;">${date}</td>
                                <td style="padding:12px;">
