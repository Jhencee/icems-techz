// ==============================================
// QRPUBLICATION.JS - QR Code Validation Module
// ==============================================
// This file handles all QR code validation functionality
// Compatible with Publication Payment Settings Modal

console.log('🔧 [Publication] QR Validation Module Loading...');
...
function loadJsQRLibrary() {
    return new Promise((resolve, reject) => {
        if (window.jsQR) {
            console.log('✅ [Publication] jsQR library already loaded');
            resolve();
            return;
        }

        console.log('📦 [Publication] Loading jsQR library from CDN...');
        const script = document.createElement('script');
        script.src = 'https://cdn.jsdelivr.net/npm/jsqr@1.4.0/dist/jsQR.min.js';
        script.onload = () => {
            console.log('✅ [Publication] jsQR library loaded successfully');
            resolve();
        };
        script.onerror = () => {
            console.error('❌ [Publication] Failed to load jsQR library');
...
function validateQRCode(file) {
    return new Promise(async (resolve, reject) => {
        console.log('🔍 [Publication] Starting QR validation for file:', file?.name);
        ...
        if (!validTypes.includes(file.type)) {
            console.warn('❌ [Publication] Invalid file type:', file.type);
        ...
        if (file.size > maxSize) {
            console.warn('❌ [Publication] File too large:', file.size, 'bytes');
        ...
        reader.onload = function(e) {
            const img = new Image();
            img.onload = function() {
                console.log('📐 [Publication] Image dimensions:', img.width, 'x', img.height);

                // Check minimum dimensions (QR codes should be at least 100x100)
                if (img.width < 100 || img.height < 100) {
                    console.warn('❌ [Publication] Image too small');
                ...
                console.log('🔎 [Publication] Scanning for QR code...');
                ...
                if (!qrCode) {
                    console.warn('❌ [Publication] No QR code detected');
                    reject('No valid QR code detected in image. Please upload an image containing a QR code.');
                    return;
                }

                // QR code detected successfully
                console.log('✅ [Publication] QR Code detected! Data:', qrCode.data);
            ...
            img.onerror = function() {
                console.error('❌ [Publication] Failed to load image');
        ...
        reader.onerror = function() {
            console.error('❌ [Publication] FileReader error');
...
function setupPaymentQRValidation() {
    console.log('🎯 [Publication] Setting up Payment QR validation...');
    
    // Look for the QR input in the payment settings form
    const qrInput = document.querySelector('#paymentSettingsForm input[name="qr_code"]');
    if (!qrInput) {
        console.warn('⚠️ [Publication] Payment QR input not found (input[name="qr_code"])');
        return;
    }

    console.log('✅ [Publication] Found payment QR input');
    ...
    if (!validationContainer) {
        ...
        console.log('✅ [Publication] Created validation container');
    }
    ...
    if (!previewContainer) {
        ...
        console.log('✅ [Publication] Created preview container');
    }
    ...
    newQrInput.addEventListener('change', async function(e) {
        ...
        console.log('📁 [Publication] File selected:', file?.name);
        ...
        try {
            ...
            console.log('✅ [Publication] QR validation successful');

        } catch (error) {
            console.error('❌ [Publication] QR validation failed:', error);
    ...
    console.log('✅ [Publication] QR validation setup complete');
}
...
function openUploadQRModal() {
    console.log('📤 [Publication] Opening Upload QR Modal...');
                ...
                <form id="uploadQRForm" style="padding: 20px 0;">
                    ...
                    <div style="margin-bottom: 20px;">
                        ...
                        <input 
                            type="text" 
                            id="qrPaymentTitle" 
                            placeholder="e.g., Publication Fee, Membership Fee"
    ...
    qrInput.addEventListener('change', async function(e) {
        ...
        console.log('📁 [Publication] File selected in upload modal:', file?.name);
        ...
        try {
            ...
            console.log('✅ [Publication] Upload modal QR validation successful');

        } catch (error) {
            console.error('❌ [Publication] Upload modal QR validation failed:', error);
    ...
    form.addEventListener('submit', function(e) {
        ...
        console.log('💾 [Publication] Uploading QR code:', {
    ...
    console.log('✅ [Publication] Upload QR Modal opened');
}

function closeUploadQRModal() {
    console.log('❌ [Publication] Closing Upload QR Modal...');
...
document.addEventListener('DOMContentLoaded', function() {
    console.log('🚀 [Publication] Initializing QR Validation Module...');
    ...
    if (openPaymentSettingsBtn) {
        console.log('✅ [Publication] Found "Create Payment" button');
        
        // Listen for clicks on the button
        openPaymentSettingsBtn.addEventListener('click', function(e) {
            console.log('🎯 [Publication] Payment Settings button clicked');
            
            // Give modal time to render, then setup validation
            setTimeout(() => {
                console.log('⏰ [Publication] Setting up QR validation after modal render...');
                setupPaymentQRValidation();
            }, 200);
        });
        
        console.log('✅ [Publication] Payment settings button listener added');
    } else {
        console.warn('⚠️ [Publication] Payment Settings button not found (id="openPaymentSettings")');
    }

    console.log('✅ [Publication] QR Validation Module initialized');
});
...
window.validateQRCodePublication = validateQRCode;
window.setupPaymentQRValidationPublication = setupPaymentQRValidation;
window.openUploadQRModalPublication = openUploadQRModal;
window.closeUploadQRModalPublication = closeUploadQRModal;

console.log('✅ [Publication] QR Validation Module loaded successfully');