...
async function loadUserProfileData() {
    ...
    try {
        ...
        if (data.success) {
            console.log('âœ… User profile loaded from DB:', data.user);
            ...
            console.warn('âš ï¸ API failed, using localStorage data instead');
            displayUserProfile(currentUser);
        }

    } catch (error) {
        console.warn('âš ï¸ No /api/auth/me endpoint, using localStorage data instead');
        // Use data already stored in localStorage from login response
        displayUserProfile(currentUser);
    }
}
